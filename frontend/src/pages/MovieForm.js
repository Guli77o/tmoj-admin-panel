...
const MovieForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        await axios.put(`${config.apiUrl}/movies/${id}`, formData);
        toast.success('Película actualizada con éxito');
      } else {
        await axios.post(`${config.apiUrl}/movies`, formData);
        toast.success('Película añadida con éxito');
      }
      
      navigate('/movies');
    } catch (error) {
      console.error('Error al guardar película:', error);
      toast.error(error.response?.data?.message || 'Error al guardar película');
      setLoading(false);
    }
  };
      ...
      <Container maxWidth="md">
        <Box sx={{ mb: 3, display: 'flex', alignItems: 'center' }}>
          ...
          <Typography variant="h4" component="h1">
            {isEditMode ? 'Editar Película' : 'Añadir Película'}
          </Typography>
        </Box>
              ...
              <Grid container spacing={3}>
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    onChange={handleChange}
                    required
                    helperText="URL de la imagen de la película (preferiblemente desde Cloudinary)"
                  />
                </Grid>
                ...
                <Grid item xs={12}>
                  <TextField
                    ...
                    onChange={handleChange}
                    required
                    helperText="URL donde se reproduce la película"
                  />
                </Grid>
